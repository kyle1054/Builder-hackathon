-- Draft plans are private. A narrow authenticated RPC controls every read/write.
alter table public.party_members drop constraint party_members_role_check;
alter table public.party_members add constraint party_members_role_check check (role in ('pilot','navigator','passenger'));
create table private.trip_plans (
 journey_id uuid primary key references public.journeys(id) on delete cascade,
 details jsonb not null,
 revision integer not null default 1
);
create table private.plan_invites (
...
create index plan_invites_journey_idx on private.plan_invites(journey_id);
create index plan_invites_recipient_idx on private.plan_invites(recipient_id);
create table private.saved_friends (
 user_id uuid not null references auth.users(id) on delete cascade,
 friend_id uuid not null references auth.users(id) on delete cascade,
 primary key(user_id,friend_id), check(user_id<>friend_id)
);
create index saved_friends_friend_idx on private.saved_friends(friend_id);
alter table private.trip_plans enable row level security;
alter table private.plan_invites enable row level security;
alter table private.saved_friends enable row level security;

create function private.trip_planner(p_action text, p_data jsonb default '{}'::jsonb) returns jsonb
...
$$;
create function public.trip_planner(p_action text,p_data jsonb default '{}'::jsonb) returns jsonb
language sql security invoker set search_path='' as $$ select private.trip_planner(p_action,p_data); $$;
revoke all on function private.trip_planner(text,jsonb),public.trip_planner(text,jsonb) from public,anon;
grant execute on function private.trip_planner(text,jsonb),public.trip_planner(text,jsonb) to authenticated;
